# Verified CIFAR-100 main experiments
...
$outputRoot = Join-Path $systemDir "reproducibility_runs\$stamp\cifar100_main"

foreach ($seed in 0,1,2,3,4) {
    Invoke-SaugRun `
        -Dataset "Cifar100" `
        -NumClasses 100 `
        -Goal "verified_C100_off_seed$seed" `
...
foreach ($seed in 0,1,2,3,4) {
    Invoke-SaugRun `
        -Dataset "Cifar100" `
        -NumClasses 100 `
        -Goal "verified_C100_speed_seed$seed" `
...
foreach ($tau in 0.4,0.5,0.6,0.7) {
    foreach ($seed in 0,1,2,3,4) {
        Invoke-SaugRun `
            -Dataset "Cifar100" `
            -NumClasses 100 `
            -Goal "verified_C100_saug_tau${tau}_seed$seed" `
